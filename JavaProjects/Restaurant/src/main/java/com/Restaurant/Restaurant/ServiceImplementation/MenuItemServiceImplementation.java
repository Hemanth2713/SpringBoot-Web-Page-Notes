...
public class MenuItemServiceImplementation implements MenuItemService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(MenuItemServiceImplementation.class);
    ...
    public List<MenuItemDto> getAllMenuItems() {
        ...
        logger.info("Fetched {} menu items.", menuItemDtos.size());
        return menuItemDtos;
    }
    ...
    public MenuItemDto getMenuItemById(Long id) {
        ...
        logger.info("Menu item with ID {} found: {}", id, menuItem.getItemName());
    ...
    public void updateMenuItem(Long id, MenuItemDto menuItemDto) {
        logger.info("Attempting to update menu item ID {}.", id);
        ...
        Menu menu = menuRepository.findById(menuItemDto.getMenuId())
                .orElseThrow(() -> {
                    logger.error("Menu ID {} not found.", menuItemDto.getMenuId());
        ...
        logger.info("Menu item ID {} updated successfully.", id);
    }

    @Override
    @Transactional
    public void toggleAvailabilityStatus(Long id) {
        logger.info("Toggling availability status for menu item ID {}.", id);
        MenuItem menuItem = menuItemRepository.findById(id)
                .orElseThrow(() -> {
                    logger.error("Menu item with ID {} not found.", id);
                    return new RuntimeException("Menu item with ID " + id + " not found.");
        ...
        menuItemRepository.save(menuItem);
        logger.info("Availability for menu item ID {} changed to {}.", id, newStatus);
    }

    @Override
    @Transactional
    public void deleteMenuItem(Long id) {
        logger.info("Attempting to delete menu item ID {}.", id);
        MenuItem menuItem = menuItemRepository.findById(id)
                .orElseThrow(() -> {
                    logger.error("Menu item with ID {} not found.", id);
                    return new RuntimeException("Menu item with ID " + id + " not found.");
    ...
    public List<MenuItemDto> getAvailableMenuItems() {
        ...
        List<MenuItemDto> dtoList = availableItems.stream()
                ...
                .toList();
        logger.info("Fetched {} available menu items.", dtoList.size());
        return dtoList;
    }
}
